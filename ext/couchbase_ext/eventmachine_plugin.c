...
/* this function is called only by timer */
    static VALUE
...
rb_em_socket_notify_readable(VALUE self)
{
    ...
    if (RTEST(event)) {
        ...
        ev->handler(ev->socket, LCB_READ_EVENT, ev->cb_data);
    }
    else {
...
rb_em_socket_notify_writable(VALUE self)
{
    ...
    if (RTEST(event)) {
        Data_Get_Struct(event, rb_em_event, ev);
        ev->handler(ev->socket, LCB_WRITE_EVENT, ev->cb_data);
    }
    else {
        rb_funcall_0(self, cb_id_detach);
    }
